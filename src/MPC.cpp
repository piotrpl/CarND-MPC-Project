...
MPC::MPC() {
  ...
  actuations_cache.resize(2);
  actuations_cache.assign(2, 0);
}
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  const double value = 25 * M_PI / 180.0;
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -value;
    vars_upperbound[i] = value;
  }
  ...
  // Freeze the control commands using previous values in order to handle latency in car control
  // Creds: https://github.com/parilo/CarND-MPC-Project
  double latency = 0.1;
  int actuations_after_delay_index = int(std::ceil(latency / dt));
  for (int i = 0; i < actuations_after_delay_index; i++) {
    vars_lowerbound[delta_start + i] = actuations_cache[0];
    vars_upperbound[delta_start + i] = actuations_cache[0];
    vars_lowerbound[a_start + i] = actuations_cache[1];
    vars_upperbound[a_start + i] = actuations_cache[1];
  }
  ...
  actuations_cache[0] = delta;
  actuations_cache[1] = a;
